...
type TicketCardProps = {
  ...
  longLabels?: boolean
  sectionLabelTranslateX?: number
  valuesTranslateY?: number
  seatValueTranslateX?: number
  sectionValueTranslateX?: number
  eventNameTranslateY?: number
  bottomLineTranslateY?: number
  redHeaderHeight?: number
  imageHeight?: number
  cardHeight?: number
...
export function TicketCard({ ticket, onUpdate, onDelete, detailsPath = '/ticket-details', notesTranslateY = 0, viewTicketTranslateY = 0, ticketDetailsTranslateY = 0, sectionRowSeatTranslateY = 0, labelsTranslateY = 0, valuesFontSize = 19.6, longLabels = false, sectionLabelTranslateX = -7, valuesTranslateY = 0, seatValueTranslateX = 6.5, sectionValueTranslateX = -6.5, eventNameTranslateY = 0, bottomLineTranslateY = 0, redHeaderHeight = 130, imageHeight = 194, cardHeight = 555, viewTicketOverlayUrl }: TicketCardProps) {
  ...
  return (
    <>
      <article className="relative w-[330px] shrink-0 overflow-hidden rounded-[18px] bg-white shadow-[0_3px_3px_rgba(0,0,0,0.15)] ring-1 ring-black/5" style={{ height: `${cardHeight}px` }}>
        <EditTicketDialog ticket={ticket} onUpdate={onUpdate} onDelete={onDelete} />
        <section className="bg-[#cb0101] px-8 pb-[27px] pt-[15px] text-white" style={{ height: `${redHeaderHeight}px` }}>
          ...
          <div className="mt-[21px] grid grid-cols-3 gap-6 text-center" style={sectionRowSeatTranslateY !== 0 ? { transform: `translateY(${sectionRowSeatTranslateY}px)` } : undefined}>
            <div className="-translate-x-[5px]">
              <p className="-translate-x-[7px] text-[13.5px] font-light" style={{ transform: `translateX(${sectionLabelTranslateX}px) translateY(${3 + labelsTranslateY}px)` }}>{longLabels ? 'Section' : 'SEC'}</p>
              <p className="mt-[-4px] -translate-x-[6.5px] font-bold tracking-tight" style={{ fontSize: `${valuesFontSize}px`, transform: `translateX(${sectionValueTranslateX}px) translateY(${valuesTranslateY}px)` }}>{ticket.section}</p>
            </div>
            <div>
              <p className="text-[13.5px] font-light" style={{ transform: `translateY(${3 + labelsTranslateY}px)` }}>{longLabels ? 'Row' : 'ROW'}</p>
              <p className="mt-[-4px] font-bold tracking-tight" style={{ fontSize: `${valuesFontSize}px`, transform: `translateY(${valuesTranslateY}px)` }}>{ticket.row}</p>
            </div>
            <div className="translate-x-[4px]">
              ...
              <p className="mt-[-4px] translate-x-[6.5px] font-bold tracking-tight" style={{ fontSize: `${valuesFontSize}px`, transform: `translateX(${seatValueTranslateX}px) translateY(${valuesTranslateY}px)` }}>{ticket.seat}</p>
            </div>
          </div>
        </section>

        <section className="relative w-[330px] overflow-hidden bg-black text-white" style={{ height: `${imageHeight}px` }}>
          {ticket.imageUrl ? (
          ...
          <div className="absolute inset-x-[10px] top-[116px] text-center drop-shadow-[0_2px_2px_rgba(0,0,0,0.8)]" style={eventNameTranslateY !== 0 ? { transform: `translateY(${eventNameTranslateY}px)` } : undefined}>
        ...
        <section className="px-6 pb-10 pt-[95px] text-center sm:px-8 sm:pb-16 sm:pt-24">
          ...
          {viewTicketOverlayUrl ? (
            <Button
              className="h-[39px] w-[280px] rounded-none bg-[#cb0101] text-[13.5px] font-normal text-white hover:bg-[#a80000]"
              style={viewTicketTranslateY !== 0 ? { transform: `translateY(${viewTicketTranslateY}px)` } : undefined}
      ...
      {viewTicketOverlayUrl && overlayOpen && (
        <div
